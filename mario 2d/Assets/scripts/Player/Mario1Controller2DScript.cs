    ...
    public class Mario1Controller2DScript : MonoBehaviour {
        ...
        void FixedUpdate (){
            ...
            if (Input.GetKey( KeyCode.A ) && Time.time - sprintTimer > sprintDelay && isGrounded || jumpedDuringSprint){
                //... then sprint
                rB2d.velocity = new Vector2(Input.GetAxis("Horizontal") * moveSpeed * Time.deltaTime * sprintMultiplier, rB2d.velocity.y);
            ...
            if (playerJumped){
                rB2d.AddForce(new Vector2(0,initialJumpForce)); //"Jump" our player up in the air!
            ...
            if (playerJumping && Time.time - jumpTimer > delayToExtraJumpForce){
                rB2d.AddForce(new Vector2(0,extraJumpForce)); //... then add some additional force to the jump
            }
        }
    }
